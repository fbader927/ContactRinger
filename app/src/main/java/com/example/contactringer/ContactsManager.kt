...
class ContactsManager(private val context: Context) {
    ...
    fun isFromSelectedContacts(phoneNumber: String?): Boolean {
        return phoneNumber?.let {
            val normalizedIncoming = normalizePhoneNumber(it)
            getSelectedContacts().any { contact ->
                normalizedIncoming.endsWith(normalizePhoneNumber(contact.number).takeLast(10))
            }
        } ?: false
    }
    ...
    fun getContactByNumber(phoneNumber: String?): Contact? {
        return phoneNumber?.let {
            val normalizedIncoming = normalizePhoneNumber(it)
            getSelectedContacts().find { contact ->
                normalizedIncoming.endsWith(normalizePhoneNumber(contact.number).takeLast(10))
            }
        }
    }
